...
void add_box( struct matrix * edges,
    ...
    //connecting to rectangle in the back
...
void add_sphere( struct matrix * edges, double cx, double cy, double cz, double r, int step ) {
  ...
  int p = 0;
  // printf("cols: %d\n", sphere_points->cols);
  for (p = 0; p<sphere_points->cols;p=p+2) {
    // printf("current column: %d\n", p);
    add_edge(edges,sphere_points->m[0][p],sphere_points->m[1][p],sphere_points->m[2][p],
              sphere_points->m[0][p+1],sphere_points->m[1][p+1],sphere_points->m[2][p+1]);
  }
}
...
void add_torus( struct matrix * edges, double cx, double cy, double cz, double r1, double r2, int step ) {
  ...
  int p = 0;
  // printf("cols: %d\n", torus_points->cols);
  for (p = 0; p<torus_points->cols;p=p+2) {
    // printf("current column: %d\n", p);
    add_edge(edges,torus_points->m[0][p],torus_points->m[1][p],torus_points->m[2][p],
              torus_points->m[0][p+1],torus_points->m[1][p+1],torus_points->m[2][p+1]);
  }
}
...
struct matrix * generate_torus( double cx, double cy, double cz, double r1, double r2, int step ) {
  ...
  while (rot <= 2 ) {
    ...
    while (cir <= 2) {
      ...
      z = -sin(M_PI*rot) * (r1 * cos(M_PI*cir) + r2) + cz;
